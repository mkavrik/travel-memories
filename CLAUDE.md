...
- [x] /upload: native `<select>` pro výběr tripu nahrazen custom dropdownem (button + `<ul>` listbox s `max-h-72 overflow-y-auto`). Native select dropdown se zobrazoval omezenou výškou (uživatel viděl jen 2 z 3 tripů); s rostoucím počtem cest by to bylo nepoužitelné. Outside-click + Escape dropdown zavřou.
- [x] Blog sidebar: `getTripShortName` už neoreznává název na prvních 2 (resp. 3) slov — vrací celý název bez `MM_YYYY` prefixu. Pro `05_2026 Ferraty na Gardě` ukazovala hlavní stránka jen "Ferraty na", detail tripu pak "Ferraty na Gardě" → nekonzistentní. Sjednoceno ve všech 3 blog page komponentách (root, [trip], [trip]/[date]).
- [x] photoCache: `sharp(decodedBuffer)` → `sharp(decodedBuffer).rotate()` v `ensureAllCaches`. Bez `.rotate()` sharp při `.resize()` EXIF Orientation tag ignoruje a při `.jpeg()` strippuje, takže JPEGy s Orientation ∈ {2..8} skončily v cache otočené (pixely v původním pořadí, ale tag už není → browser nemá co aplikovat). HEIC fotky problém neměly, protože `heic-convert` rotaci bakuje do pixelů při dekódování. Postiženo bylo 5 fotek v Southwest USA tripu (z 305 originálů); cache pro ně zregenerována, Supabase invalidován. Diagnostické skripty `scripts/scan-orientation.mjs` (sken všech originálů na Orientation != 1), `scripts/fix-rotation.mjs` (cílená re-konverze + Supabase invalidace) a `scripts/verify-rotation.mjs` (ověření výsledných rozměrů + stripped tagu).
